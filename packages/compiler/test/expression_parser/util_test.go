...
var parser = expression_parser.NewParser(expression_parser.NewLexer(), false)

func parseAction(expression string) *expression_parser.ASTWithSource {
	return parser.ParseAction(expression, getFakeSpan(""), 0)
}

func getFakeSpan(fileName string) *util.ParseSourceSpan {
	if fileName == "" {
		fileName = "test.html"
	}
	file := util.NewParseSourceFile("", fileName)
	location := util.NewParseLocation(file, 0, 0, 0)
	return util.NewParseSourceSpan(location, location, location, nil)
}
...
func (u *Unparser) VisitEmptyExpr(ast *expression_parser.EmptyExpr, context interface{}) interface{} {
	// EmptyExpr should serialize as empty string (for trailing commas, etc.)
	// But in unparse, we might want to show it differently
	// For now, just do nothing (empty string)
	return nil
}
...
func (u *Unparser) VisitKeyedRead(ast *expression_parser.KeyedRead, context interface{}) interface{} {
	...
	u.expression.WriteString("[")
	// EmptyExpr.Visit returns nil and doesn't call visitor method, so we need to handle it
	if _, ok := ast.Key.(*expression_parser.EmptyExpr); ok {
		// EmptyExpr should serialize as empty string (for trailing commas, etc.)
