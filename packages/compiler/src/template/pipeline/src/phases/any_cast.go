...
func DeleteAnyCasts(job *compilation.CompilationJob) {
	for _, unit := range job.GetUnits() {
		...
		for op := unit.GetCreate().Head(); op != nil; op = op.Next() {
			expression.TransformExpressionsInOp(op, removeAnys, expression.VisitorContextFlagNone)
		}
...
func removeAnys(e output.OutputExpression, flags expression.VisitorContextFlag) output.OutputExpression {
	if invoke, ok := e.(*output.InvokeFunctionExpr); ok {
		if lexicalRead, ok := invoke.Fn.(*expression.LexicalReadExpr); ok && lexicalRead.Name == "$any" {
